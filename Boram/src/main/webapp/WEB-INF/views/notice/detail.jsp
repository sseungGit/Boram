...
<div class="container" style="margin-bottom: 300px;">
	...
	<h2>${dto.title }</h2>
	<br>
	<span>작성자 : ${dto.writer } 조회수 : ${dto.viewCount }<br> 
	작성일 : ${dto.regdate }</span>
	<div class="content">${dto.content }</div>
	<a  class="btn btn-outline-dark" href="list.do">목록보기</a>
	<c:if test="${dto.writer eq id }">
		<a  class="btn btn-outline-dark" href="updateform.do?num=${dto.num }">수정</a>
		<a  class="btn btn-outline-dark" href="delete.do?num=${dto.num }">삭제</a>
	</c:if>
	<br><br>
	<!-- 댓글 목록 -->
	<div class="comments">
		<ul>
			<c:forEach var="tmp" items="${commentList }">
				<c:choose>
					...
					<c:otherwise>
						<%-- 만일 댓글의 글번호가 댓글의 그룹번호와 같다면 --%>
						<c:if test="${tmp.num eq tmp.comment_group }">
							<li id="reli${tmp.num }">
						</c:if>
						<%-- 
							만일 댓글의 글번호가 댓글의 그룹번호와 다르다면
							왼쪽 패딩을 50px 주어서 들여쓰기 효과를 주고
							화살표 아이콘을 출력한다.
						 --%>
						<c:if test="${tmp.num ne tmp.comment_group }">
							<li id="reli${tmp.num }" style="padding-left:50px;">
								...
								<dl>
									<dt>
										...
										<%-- 만일 프로필 이미지가 댓글의 그룹번호와 다르다면(대댓글이라면) 누구를 향한 댓글인지 출력하기 --%>
										<c:if test="${tmp.num ne tmp.comment_group }">
											@<i>${tmp.target_id }</i>
										</c:if>
										<span>${tmp.regdate }</span>
										<%-- 답글 링크를 눌렀을 때 해당댓글을 글번호를 얻어오기 위해 data-num 속성에 댓글의 번호 넣어두기 --%>
										<a data-num="${tmp.num }" href="javascript:" class="reply-link">답글</a>
										<%-- 만일 로그인을 했고 작성자가 로그인 된 사용자와 같다면 수정, 삭제 링크를 출력한다. --%>
									...
									<dd>
										<%-- 
											댓글은 textarea 로 입력받았기 때문에 tab, 공백, 개행 기호도 존재한다.
								...
								<%-- 
									댓글의 댓글 폼은 미리 만들어서 숨겨놓았다가 답변 링크를 클릭하면 보이도록 한다.
							...
							<%-- 
								만일 글 작성자가 로그인한 작성자라면 (본인이 작성한 댓글이라면) 댓글 수정 폼도 미리 준비해 놓고
								숨겨둔다.		
								javascript 에서 폼을 바로 선택할 수 있도록 댓글 번호를 조합해서 아이디를 부여해 놓았다.						
							 --%>
							<c:if test="${tmp.writer eq id }">
	...
	<%-- 
		...
		일단 숨겨두었다가 javascript 에서 조건부로 보이게 하고 다시 숨기고 하는 작업을 한다.
